...
const MINIMAL_JPEG: &[u8] = &[
    ...
    0x00, 0x08, 0x01, 0x01, 0x00, 0x00, 0x3F, 0x00, 0x7B, 0x2A, 0xA6, 0xF7, 0xFF, 0xD9,
];
...
async fn test_media_list() {
    ...
    eprintln!(
        "Media list response keys: {:?}",
        resp.get("data")
            .map(|_| "data")
            .or_else(|| resp.get("results").map(|_| "results"))
            .or_else(|| {
                eprintln!("Full response: {:#?}", serde_json::to_string(&resp).ok());
                None
            })
    );
    let results = resp
    ...
    assert!(
        results.is_some(),
        "should have results. Full response: {:#?}",
        serde_json::to_string(&resp).ok()
    );

    assert!(
        results.is_some(),
        "response should have results array. Full response: {:#?}",
        serde_json::to_string(&resp).ok()
    );
